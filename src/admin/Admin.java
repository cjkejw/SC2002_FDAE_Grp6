...
public class Admin extends FilterDisplayStaffList implements DisplayingStaffList, PersonInterface, PaymentInterface, PersonDependency, BranchDependency {
	...
	public void checkingManagerCount() {
		...
		while (curManager > needManager) {
			this.personTable = PersonInterface.getPersonTable(); 
			System.out.println("You need to remove " + (curManager - needManager) + " mangers from " + branchName); 
			StaffAddRemoval func = new StaffAddRemoval(); 
			System.out.println("Enter the Staff ID of the Manager to be removed"); 
			String staffID = scan.next();
			scan.nextLine(); 
			if (personTable.get(staffID).getRole().equals("M")) {
				Hashtable <String, Person> personTable = func.removeStaffAcc(this.personTable, staffID);
				if (personTable != null) {
					this.personTable = personTable; 
					curManager--; 
				}
				...
				System.out.println("Staff member is not a Manager! Choose a manager\n"); 
			}
			PersonInterface.exportPersonTable(this.personTable);
		}
	}
	...
	public void addPaymentMethod() {
		...
		for(String key : paymentTable.keySet()) {
			if(key.equalsIgnoreCase(payment)) {
				...
				paymentTable.put(key,  methods); //puts the updated list of payment methods back in the payment hashtable according to the branchName
					//break;
			}else {
	...
	public void displayStaffList(String branchName){
		...
		System.out.println("\nThere are : \n\t" + ManagerNum + " Managers\n\t" + StaffNum + " Regular Staff Members");
		}
		else {
